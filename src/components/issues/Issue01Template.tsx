            ...
            <section className="grid gap-8 lg:grid-cols-[minmax(0,420px)_minmax(0,1fr)]">
              <div className="space-y-4">
                <div className="overflow-hidden rounded-[26px] border border-black/10 bg-black p-4 shadow-[0_26px_60px_rgba(0,0,0,0.14)]">
                  {coverImage ? (
                    <img src={coverImage} alt={`BLACKFILES Issue ${String(issue.number).padStart(2, "0")} cover`} className="w-full object-contain" loading="eager" />
                  ) : (
              ...
              <div className="space-y-6 pt-2">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {issue.tags.map((tag) => (
                      <span key={tag} className="rounded border border-black/10 bg-white/55 px-3 py-1.5 font-mono text-[11px] uppercase tracking-[0.2em] text-black/70">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex flex-wrap gap-3">
                  {shareTargets.map(({ label, href, icon: Icon }) => (
                    <a key={label} href={href} target="_blank" rel="noreferrer noopener" className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/70 px-4 py-2 font-mono text-[11px] uppercase tracking-[0.24em] text-black/70 transition-colors hover:border-[#ff8b4d]/45 hover:text-[#ff6a3d]">
            ...
            <div className="sticky top-24 space-y-5">
                  ...
                  {tocSections.map((section, index) => (
                    <li key={section.id} className="border-b border-white/10 pb-3 last:border-0">
                      <a href={`#${section.id}`} className="flex items-start gap-3">
                        <span className="min-w-6 font-mono text-[15px] text-[#ff6a3d]">
                          {index + 1}
                        </span>
                        <span className="text-[15px] leading-6 text-white/88">
                          {section.title}
                        </span>
                      </a>
                    </li>
              ...
              <section className="rounded-2xl bg-[#111111] p-5 text-[#f2e8d8] shadow-[0_30px_60px_rgba(0,0,0,0.18)]">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {shareTargets.map(({ label, href, icon: Icon }) => (
                    <a key={label} href={href} target="_blank" rel="noreferrer noopener" className="inline-flex items-center gap-2 rounded-full border border-white/10 px-3 py-2 font-mono text-[10px] uppercase tracking-[0.24em] text-white/80 transition-colors hover:border-[#ff8b4d]/50 hover:text-[#ff8b4d]">
